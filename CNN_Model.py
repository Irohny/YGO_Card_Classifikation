...
class CNN_Model():
    ...
    def build_CNN(self):
        ...
        self.model = keras.Sequential([
            layers.experimental.preprocessing.Rescaling(1/255, input_shape=(self.xPixel, self.yPixel, self.zLayer)),
    ...
    def train_model(self, Data, Label):
        ...
        test_loss = np.zeros(self.NEpoch)
        ...
        for epoch in range(self.NEpoch):
            ...
            test_loss[epoch] = np.mean(loss)
            self.acc_metric.reset_states()
            print('Test Accuancy: %f \n' % (test_acc[epoch]))
               
            if(epoch > self.MinEpoch and test_acc[epoch] == test_acc.max()):
                self.model.save('./Model/CNN_Model_Fluffal_ACC_%s_LR_%s_BS_%s_DO_%s_Reg_%s_Ro_%s_Zo_%s_KS_%s.h5' %(np.round(test_acc[epoch], 2), self.lr, self.BatchSize, self.dropout, self.reg, self.Rotation, self.Zoom, self.KS))
               
            # early break      
            #if(epoch > self.MinEpoch):
            #      con = np.mean(test_acc[epoch-self.MinEpoch:epoch] - test_acc[epoch]*np.ones(self.MinEpoch))
            #      if np.abs(con) <= self.BreakCon:
            #                  return train_acc[0:epoch], test_acc[0:epoch], test_loss[0:epoch], train_loss[0:epoch]
                        
        
        return train_acc[0:epoch], test_acc[0:epoch], train_loss[0:epoch], test_loss[0:epoch] 
